...
public class UsedColumnExtractorVisitor implements ExpressionVisitor, FromItemVisitor, ItemsListVisitor, SelectVisitor, SelectItemVisitor {
	...
	private class State {
		...
		boolean isExistsState;
		boolean isNextStateExists;
		...
		private State(State copyState) {
			...
			isExistsState = false;
			isNextStateExists = false;
			if (copyState.isNextStateExists)
			{
				isExistsState = true;
			}
		}
...
//		}
		// If it is outer select, don't add, otherwise do add
		if (!((currentState.isOuterState || currentState.isExistsState) && currentState.phase == SelectPhase.SELECT))
		{
			addUsedColumn(arg0.getColumnName(), tableName);
		}
	}
	...
	public void visit(ExistsExpression arg0) {
		...
		{
			this.currentState.isNextStateExists = true;
			arg0.getRightExpression().accept(this);
		}
	}
...
//			useAllColumns(null);
//		}
		if (!(currentState.isOuterState || currentState.isExistsState)) {
			sourceAllColumns(null);
			useAllColumns(null);
		}
	}
